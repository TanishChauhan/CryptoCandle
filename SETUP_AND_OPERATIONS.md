...
**Tracked symbols (default):** `BTCUSDT`, `ETHUSDT`
...
| Windows `./data` permission errors | Spark writes as root; Airflow as uid 50000 | `.\scripts\fix-data-permissions.ps1` then retry DAG |
| Airflow UI logs **403 FORBIDDEN** | `secret_key` / log path mismatch | Set `AIRFLOW_SECRET_KEY` in `.env`, recreate webserver+scheduler; use terminal logs or `./airflow/logs` |
...
**Free disk space manually** (Parquet + checkpoints + Postgres analytics; optional Kafka):

```powershell
.\scripts\free-disk-space.ps1
.\scripts\free-disk-space.ps1 -IncludeKafka   # also wipe Kafka topic messages
```

**Wipe ALL pipeline data** (full reset before fresh start):

```powershell
.\scripts\reset-all-data.ps1
```
...
| Location | Contents |
|----------|----------|
| `./data/raw/trades/` | Spark raw Parquet (largest on-disk use) |
| `./data/raw/trades_compacted/` | Batch-compacted Parquet (Airflow path) |
| `./data/checkpoints/stream_trades/` | Spark Structured Streaming checkpoints |
| Docker volume `postgres_data` | PostgreSQL (`analytics.*` tables) |
| Kafka container | Topic messages (`crypto_trades`, `dead_letter_queue`; 1-day retention on trades) |
| `./debug-*.log` | Optional Spark debug logs |

**Reset local state (manual alternative):**

```powershell
docker compose down -v
Remove-Item -Recurse -Force data\raw, data\checkpoints -ErrorAction SilentlyContinue
docker compose up -d
```
